...
public:
    ...
    static constexpr RiderMode mode_empty = 0b00001;
    static constexpr RiderMode mode_hash = 0b00010;
    static constexpr RiderMode mode_full = 0b00100;
    static constexpr RiderMode mode_deleted = 0b01000;
    static constexpr RiderMode mode_undefined = 0b10000;
    ...
    static constexpr HashType log_batch_size = 5;
    static constexpr HashType batch_size = (1 << log_batch_size);
...
Metadata::Rider<mode>::Rider(BucketHashType* metadata, HashType hash, HashType hash_mask)
    : metadata_(metadata),
      ...
      initial_hash_(hash & hash_mask),
      current_hash_(hash & hash_mask),
      batch_(_mm256_setzero_si256()) {}
...
HashType Metadata::Rider<mode>::get_current_hash() const {
    return (current_hash_ - 1) & hash_mask_;
}

template <Metadata::RiderMode mode>
std::pair<HashType, Metadata::RiderMode> Metadata::Rider<mode>::next() {
    if (!((current_hash_ - initial_hash_) & (batch_size - 1))) {
        batch_ = _mm256_loadu_si256(reinterpret_cast<__m256i*>(metadata_ + current_hash_));
    }
    do {
        HashType chunk_shift = ((current_hash_ - initial_hash_) & (batch_size - 1));
        HashType hash_delta = std::min(batch_size, hash_mask_ - current_hash_);
        RiderMode return_mode = mode_undefined;

        if constexpr ((mode & Metadata::mode_empty) != 0) {
            const auto match = _mm256_set1_epi8(get_empty());
            uint32_t mask = static_cast<uint32_t>(_mm256_movemask_epi8(_mm256_cmpeq_epi8(match, batch_)));
            if (mask >> chunk_shift != 0) {
                uint8_t ctz = __builtin_ctz(mask >> chunk_shift);
                if (ctz < hash_delta) {
                    hash_delta = ctz;
                    return_mode = Metadata::mode_empty;
                }
            }
        }
        if constexpr ((mode & Metadata::mode_deleted) != 0) {
            const auto match = _mm256_set1_epi8(get_deleted());
            uint32_t mask = static_cast<uint32_t>(_mm256_movemask_epi8(_mm256_cmpeq_epi8(match, batch_)));
            if (mask >> chunk_shift != 0) {
                uint8_t ctz = __builtin_ctz(mask >> chunk_shift);
                if (ctz < hash_delta) {
                    hash_delta = ctz;
                    return_mode = Metadata::mode_deleted;
                }
            }
        }
        if constexpr ((mode & Metadata::mode_full) != 0) {
            const auto match_empty = _mm256_set1_epi8(get_empty());
            const auto match_deleted = _mm256_set1_epi8(get_deleted());
            uint32_t mask = static_cast<uint32_t>(_mm256_movemask_epi8(
                _mm256_cmpeq_epi8(_mm256_cmpeq_epi8(match_empty, batch_), _mm256_cmpeq_epi8(match_deleted, batch_))));
            if (mask >> chunk_shift != 0) {
                uint8_t ctz = __builtin_ctz(mask >> chunk_shift);
                if (ctz < hash_delta) {
                    hash_delta = ctz;
                    return_mode = Metadata::mode_full;
                }
            }
        }
        if constexpr ((mode & Metadata::mode_hash) != 0) {
            const auto match = _mm256_set1_epi8(get_hash(initial_hash_));
            uint32_t mask = static_cast<uint32_t>(_mm256_movemask_epi8(_mm256_cmpeq_epi8(match, batch_)));
            if (mask >> chunk_shift != 0) {
                uint8_t ctz = __builtin_ctz(mask >> chunk_shift);
                if (ctz < hash_delta) {
                    hash_delta = ctz;
                    return_mode = Metadata::mode_hash;
                }
            }
        }
        current_hash_ = (current_hash_ + hash_delta) & hash_mask_;
        if (return_mode != mode_undefined) {
            HashType last_hash = current_hash_;
            current_hash_ = (current_hash_ + 1) & hash_mask_;
            return {last_hash, return_mode};
        } else {
            current_hash_ = (current_hash_ + 1) & hash_mask_;
            batch_ = _mm256_loadu_si256(reinterpret_cast<__m256i*>(metadata_ + current_hash_));
        }
...
void HashMap<KeyType, ValueType, Hash>::erase_generic(HashType index) {
    ...
    if (log_capacity_ > Metadata::log_batch_size && size_ < ((1ull << log_capacity_) >> 3ull)) {
        resize(log_capacity_ - 1);
    }
}
...
std::pair<bool, HashType> HashMap<KeyType, ValueType, Hash>::find_allow_deleted_generic(const KeyType& key) const {
    HashType hash = hasher_(key);
    Metadata::Rider<Metadata::mode_empty | Metadata::mode_hash | Metadata::mode_deleted> rider(
        metadata_, hash, (1 << log_capacity_) - 1);
    bool not_found_empty_position = true;
    HashType empty_position = 0;
    do {
        std::pair<HashType, Metadata::RiderMode> p = rider.next();
        switch (p.second) {
            case Metadata::mode_empty:
...
void HashMap<KeyType, ValueType, Hash>::copy_from_iterators_generic(IteratorType l, IteratorType r,
    ...
    metadata_ = new Metadata::BucketHashType[(1 << log_capacity_) + Metadata::batch_size];
    memset(metadata_, Metadata::get_empty(), (1 << log_capacity_) + Metadata::batch_size);
...
HashMap<KeyType, ValueType, Hash>::HashMap(Hash hasher) : hasher_(hasher) {
    log_capacity_ = Metadata::log_batch_size;
    metadata_ = new Metadata::BucketHashType[(1 << log_capacity_) + Metadata::batch_size];
    memset(metadata_, Metadata::get_empty(), (1 << log_capacity_) + Metadata::batch_size);
    data_ = new KeyValType[1 << log_capacity_];
}
...
HashMap<KeyType, ValueType, Hash>::HashMap(const HashMap& other)
    ...
    metadata_ = new Metadata::BucketHashType[(1 << log_capacity_) + Metadata::batch_size];
    memcpy(metadata_, other.metadata_, (1 << log_capacity_) + Metadata::batch_size);
    data_ = new KeyValType[1 << log_capacity_];
...
HashMap<KeyType, ValueType, Hash>::HashMap(std::initializer_list<KeyValType> l, Hash hasher) : hasher_(hasher) {
    ...
    ++log_capacity_;
    log_capacity_ = std::max(Metadata::log_batch_size, log_capacity_);
...
const HashMap<KeyType, ValueType, Hash>& HashMap<KeyType, ValueType, Hash>::operator=(const HashMap& other) {
    if constexpr (std::is_copy_constructible<Hash>::value) {
        ...
        size_ = other.size_;
        full_ = other.full_;
        log_capacity_ = std::max(Metadata::log_batch_size, other.log_capacity_);
